...
To be compatible with the [A2A "text" parts](https://a2a-protocol.org/dev/specification/#651-textpart-object), the agent must have a `messages` key in state. For multi-turn conversations, reuse message.contextId from the prior response. (It’s omitted on the first message only.) Here's an example:
...
async def send_message(session, port, assistant_id, text, context_id=None, trace_thread_id=None):
    ...
    # A2A multi-turn continuity: reuse contextId across turns/agents
    if context_id:
    ...
    # Optional (LangSmith UI grouping): not part of the A2A spec
    if trace_thread_id:
        payload["metadata"] = {"thread_id": trace_thread_id}
    ...
    return extract_text(result), returned_context_id
...
async def simulate_conversation():
    ...
    # Optional: stable trace grouping even before contextId exists
    trace_thread_id = str(uuid.uuid4())
    ...
    async with aiohttp.ClientSession() as session:
        for i in range(3):
            ...
            message, context_id = await send_message(
                ...
                trace_thread_id=(context_id or trace_thread_id),
            )
